...
dummy_reg=0 #use dummy volume as intermediate registration from functional to anatomical data
...
while [ $# -gt 0 ] ; do
  case $1 in
    ...
    --dummy_reg | --dummy-reg) dummy_reg=1; ;;
...
if [ -f ${output_folder}/func/${FULLID_folder}/${FULLID_file}_BNA_timeseries.txt ]; then
  ...
  sbatch --wait ${scriptfolder}/steps/func_sdc.bash ${anatomical_brain} ${fmri} ${scriptfolder} ${FULLID_file} ${FULLID_folder} ${remove_vols} || print_error ${output_folder} ${FULLID_folder}
  fi
  echo "  Performing FEAT..." &&\
  sbatch --wait ${scriptfolder}/steps/feat.bash ${anatomical_noneck} ${anatomical_brain} ${fmri} ${scriptfolder} ${remove_vols} ${skip_slice_time} ${slice_order_file} ${FULLID_folder} ${dummy_reg} &&\
  ...
  sbatch --wait ${scriptfolder}/steps/tempfilt.bash ${FULLID_folder} ${FULLID_file} ${advanced_tempfilt} &&\
